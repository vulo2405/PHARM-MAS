...
compartments = {}
max_items_allowed = float('inf')  # Set the default maximum number of items
        ...
        if item:
            ...
            if quantity is not None:
                if quantity > max_items_allowed:
                    ...
                    message = f"{quantity} {item}(s) stored in {selected_compartment} (Max: {max_items_allowed})"
...
if __name__ == "__main__":
    ...
    set_compartments_button = tk.Button(root, text="Set Number of Compartments", command=set_number_of_compartments)
    set_compartments_button.pack(pady=10)

    set_max_items_button = tk.Button(root, text="Set Maximum Items", command=set_max_items_allowed)
    set_max_items_button.pack(pady=10)

    store_button = tk.Button(root, text="Store", command=show_empty_compartments)
    store_button.pack(pady=10)

    retrieve_button = tk.Button(root, text="Retrieve", command=retrieve_item)
